...
export default function Posters() {
  ...
  // Filter only Posters
  const posters = products.filter(p => p.category === 'Posters' && !p.hidden);
  ...
  return (
    <div className="category-page container" style={{ paddingTop: '30px', paddingBottom: '60px' }}>
      ...
      <div className="cat-hero card" style={{
        background: 'linear-gradient(135deg, #FFE8EA 0%, #FFF5F6 100%)',
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '600px' }}>
          <span className="badge badge-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontWeight: '700' }}>
            <Sparkles size={12} /> Aesthetic Inspiration
          </span>
          <h1 style={{ fontSize: '2.8rem', margin: '15px 0 10px', color: 'var(--text-primary)', fontWeight: '800' }}>
            Affirmation Wall Posters & Art
          </h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '1.05rem', lineHeight: '1.6' }}>
            Bring good energy to your bedroom or workspace. Heavyweight matte prints, positive quotes, nursery room illustrations, and hand-drawn card collections.
          </p>
        </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '0.95rem', fontWeight: '600', color: 'var(--text-secondary)' }}>
          Showing {sortedPosters.length} adorable posters
        </div>
        ...
        <div className="card" style={{ padding: '20px', marginBottom: '30px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <div style={{ maxWidth: '300px' }}>
            ...
            <input 
              ...
              aria-label="Filter posters by maximum price"
            />
          </div>
        </div>
      )}
      ...
      {sortedPosters.length === 0 ? (
        <div className="card text-center" style={{ padding: '50px 20px' }}>
          ...
          <h3 style={{ margin: '15px 0 10px' }}>No Posters found in this price range</h3>
          <p style={{ color: 'var(--text-muted)' }}>Try raising your budget limit to explore our premium heavy-weight matte prints.</p>
          <button onClick={() => setPriceLimit(30)} className="btn btn-primary" style={{ marginTop: '10px' }}>Reset Price Filter</button>
        </div>
      ) : (
        <div className="grid-responsive" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '30px' }}>
          {sortedPosters.map(prod => (
