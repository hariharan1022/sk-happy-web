...
export default function BirthdayKits() {
  ...
  // Filter only Birthday Kit
  const kits = products.filter(p => p.category === 'Birthday Kit' && !p.hidden);

  const filteredKits = kits.filter(p => {
  ...
  return (
    <div className="category-page container" style={{ paddingTop: '30px', paddingBottom: '60px' }}>
      ...
      <div className="cat-hero card" style={{
        background: 'linear-gradient(135deg, #EBFDFB 0%, #FFFDF5 100%)',
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '600px' }}>
          <span className="badge badge-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontWeight: '700' }}>
            <Sparkles size={12} /> Party Celebrations
          </span>
          <h1 style={{ fontSize: '2.8rem', margin: '15px 0 10px', color: 'var(--text-primary)', fontWeight: '800' }}>
            Deluxe Birthday Kits & Bundles
          </h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '1.05rem', lineHeight: '1.6' }}>
            Make every celebration unforgettable. Coordinated banners, high-density balloons, cute birthday crowns, cake toppers, and organic goodies packaged in dream themes.
          </p>
        </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '0.95rem', fontWeight: '600', color: 'var(--text-secondary)' }}>
          Showing {sortedKits.length} matching Birthday kits
        </div>
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          <select 
            ...
            aria-label="Sort birthday kits by"
          >
        ...
        <div className="card" style={{ padding: '20px', marginBottom: '30px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <div style={{ maxWidth: '300px' }}>
            ...
            <input 
              ...
              aria-label="Filter birthday kits by maximum price"
            />
          </div>
        </div>
      )}
      ...
      {sortedKits.length === 0 ? (
        <div className="card text-center" style={{ padding: '50px 20px' }}>
          ...
          <h3 style={{ margin: '15px 0 10px' }}>No Birthday Kits found in this price range</h3>
          <p style={{ color: 'var(--text-muted)' }}>Try raising your budget limit to explore our coordinated deluxe kits.</p>
          <button onClick={() => setPriceLimit(50)} className="btn btn-primary" style={{ marginTop: '10px' }}>Reset Price Filter</button>
        </div>
      ) : (
